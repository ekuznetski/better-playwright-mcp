...
export class SmartOutlineGenerator {
    highPriorityTypes = new Set(['heading', 'button', 'link', 'searchbox', 'navigation', 'main', 'form', 'article', 'section']);
    mediumPriorityTypes = new Set(['list', 'listitem', 'textbox', 'checkbox', 'radio', 'select', 'table']);
    lowPriorityTypes = new Set(['generic', 'separator', 'img', 'text']);
    /**
     * Phase 1: Build complete page structure
     */
    buildPageStructure(lines) {
        const nodes = new Map();
    ...
    calculatePriority(node) {
        ...
        if (node.content) {
            ...
            if (node.content.length > 10)
                score += 1;
            // Interactive element
            if (node.content.includes('[cursor=pointer]'))
                score += 1;
            // Has URL
            if (node.content.includes('/url:'))
                score += 1;
            // Is a heading
            if (node.content.includes('[level='))
                score += 2;
        }
        // Special cases
        if (node.type === 'searchbox')
            score = 10; // Always highest
     ...
     */
    detectRepetitiveGroups(structure) {
        const typeIndentMap = new Map();
     ...
     */
    generateSmartOutline(structure, options) {
        const output = [];
        ...
        skeleton.forEach(line => {
            ...
            if (match)
                usedLines.add(parseInt(match[1]));
        });
        ...
        const priorityNodes = Array.from(structure.nodes.values())
            .filter((n) => !usedLines.has(n.lineNumber) && !n.isRepetitive)
     ...
     */
    extractSkeleton(structure, maxLines) {
        const output = [];
        ...
        while (queue.length > 0 && lines < maxLines) {
            ...
            queue.push(...node.children.filter((c) => c.priority >= 5));
        }
        return output;
    }
    /**

...
export class PlaywrightServer {
    ...
    constructor(port = parseInt(process.env.PORT || '3102')) {
        this.port = port;
    ...
    registerRoutes() {
        ...
        this.app.post('/api/pages/:pageId/hover', async (req, res) => {
            try {
                const { pageId } = req.params;
                const { ref, element } = req.body;
        ...
        // Press key
        this.app.post('/api/pages/:pageId/press', async (req, res) => {
            try {
                const { pageId } = req.params;
                const { key } = req.body;
        ...
        // File upload
        this.app.post('/api/pages/:pageId/upload', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        // Handle dialog
        this.app.post('/api/pages/:pageId/dialog', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        // Navigate back
        this.app.post('/api/pages/:pageId/back', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        // Navigate forward
        this.app.post('/api/pages/:pageId/forward', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        // Scroll to bottom
        this.app.post('/api/pages/:pageId/scroll-bottom', async (req, res) => {
            try {
                const { pageId } = req.params;
                const { ref } = req.body;
        ...
        // Scroll to top
        this.app.post('/api/pages/:pageId/scroll-top', async (req, res) => {
            try {
                const { pageId } = req.params;
                const { ref } = req.body;
        ...
        // Wait for timeout
        this.app.post('/api/pages/:pageId/wait-timeout', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        // Wait for selector
        this.app.post('/api/pages/:pageId/wait-selector', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        this.app.post('/api/pages/:pageId/outline', async (req, res) => {
            try {
                const { pageId } = req.params;
        ...
        // Search snapshot
        this.app.post('/api/pages/:pageId/search', async (req, res) => {
            try {
                const { pageId } = req.params;
    ...
    async ensureBrowser() {
        ...
        if (!this.persistentContext) {
            ...
            this.persistentContext = await playwright.chromium.launchPersistentContext(this.userDataDir, {
    ...
    async hover(pageId, ref, element) {
        const pageInfo = this.pages.get(pageId);
        if (!pageInfo) {
            throw new Error(`Page ${pageId} not found`);
        }
    ...
    async uploadFiles(pageId, ref, files) {
        const pageInfo = this.pages.get(pageId);
        if (!pageInfo) {
            throw new Error(`Page ${pageId} not found`);
        }
    ...
    async handleDialog(pageId, accept, text) {
        const pageInfo = this.pages.get(pageId);
        if (!pageInfo) {
            throw new Error(`Page ${pageId} not found`);
        }
    ...
    generateOutlineOld(snapshot) {
        ...
        const isHighPriorityGroup = (group) => {
        ...
        for (let i = 0; i < lines.length && lineCount < maxLines; i++) {
            ...
            const group = listGroups.find(g => g.firstLine === i && !processedGroups.has(g));
